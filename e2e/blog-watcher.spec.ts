...
test.describe('VRT on blogs', () => {
    test('has title', async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot('blog_index_page.png', { threshold: 0, maxDiffPixels: 100, fullPage: true });
    });
    ...
    test('navigate to programming categories', async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot('blog_programming_page.png', { threshold: 0, maxDiffPixels: 100, fullPage: true });
    });
    ...
    test('navigate to blog detail', async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot('blog_detail_page.png', { threshold: 0, maxDiffPixels: 100, fullPage: true });
    });
});